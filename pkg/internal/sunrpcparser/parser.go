...
// Procedure arguments are not decoded (RPCSEC_GSS payloads are opaque at the wire).
...
// ReplyInfo holds minimal accepted-reply status from an ONC RPC REPLY.
...
func parseReply(body []byte) (*ReplyInfo, bool, error) {
	...
	if replyStat != replyAccepted {
		return &ReplyInfo{AcceptStat: replyStat}, true, nil
	}
...
func validAuthFlavor(flavor uint32) bool {
	switch flavor {
	case authNull, authUnix, authShort, authDES, authRPCSECgss:
		return true
	default:
		return false
	}
